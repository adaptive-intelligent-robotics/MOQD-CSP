...
if __name__ == '__main__':
    ...
    tic = time.time()
    for el in tqdm(list_of_10):
        relax_results = optimizer_ref.relax(el, fmax=0.2, steps=n_relax_steps, verbose=False)
    times_ase.append(time.time() - tic)
    ...
    plt.scatter([1, 10], times_marta, label="marta")
    plt.scatter([1, 10], times_ase, label="ase")
    plt.legend()
    plt.show()
