...
from csp_elites.map_elites.elites_utils import (
    __centroids_filename as make_centroid_filename,
)
...
class ExperimentOrganiser:
    def __init__(self):
        ...
        self.experiment_location = (
            pathlib.Path(__file__).parent.parent / ".experiment.nosync"
        )
    ...
    def is_experiment_valid(self, folder_name: str):
        list_of_archives = [
            name
            for name in os.listdir(f"{self.experiment_directory_path / folder_name}")
            if not os.path.isdir(self.experiment_directory_path / folder_name / name)
    ...
    def get_centroid_name(self, folder_name: str):
        with open(
            self.experiment_directory_path / folder_name / "experiment_parameters.pkl",
            "rb",
        ) as file:
        ...
        if folder_name == "20230813_23_55_TiO2_constrained_qd_bg_shear":
            bd_minimum_values, bd_maximum_values = (
                experiment_parameters["bd_minimum_values"],
    ...
    def get_config_data(self, date: str):
        ...
        config_files_1 = [
            name
            for name in os.listdir(config_folder_1)
        ...
        if os.path.isdir(self.repo_location / "configs" / next_day):
            ...
            config_files_2 = [
                name
                for name in os.listdir(config_folder_2)
    ...
    def map_config_data_to_experiment(
        self, experiments: List[str], config_for_csv: Dict[str, Any], tag
    ):
        mapped_configs = {}
        ...
        new_cols = (
            temp_cols[index : index + 1] + temp_cols[:index] + temp_cols[index + 1 :]
        )
        df = df[new_cols]
            ...
            if self.is_experiment_valid(folder):
                max_archive = max(
                    [
                        ...
                        for name in os.listdir(self.experiment_directory_path / folder)
                        if (
...
def load_config_list_into_csv(date: str):
    ...
    df.to_csv(
        pathlib.Path(__file__).parent.parent
        / ".experiment.nosync"
        / f"{date}_list_of_configs_no_exp_mapping.csv"
    )
