...
def force_mut_only(plot_individually=True, gaussian=True, lr_step_only= True, lr_step_only_simple=True,
                   compare=True):
    report_generator = ReportPlotGenerator(
        path_to_experiments=pathlib.Path(
            __file__).parent.parent.parent / ".experiment.nosync/report_data/4_force_mutation_only",
        plot_labels=None,
        title_tag=None
    )
    # if gaussian:
    ...
    if lr_step_only_simple:
        report_generator.plot_combined_figure_for_report(
            folder_names=[
                "no_gaussian_lr-3_simple", "no_gaussian_lr-1_simple", "no_gaussian_lr1_simple"
        ...
        report_generator.plot_combined_figure_for_report(
            folder_names=[
                "no_gaussian_lr1_simple", "no_gaussian_lr1"
                # "lr1_simple_batch_10_no_relax"
            ],
    ...
    if lr_step_only:
        report_generator.plot_combined_figure_for_report(
            folder_names=[
                "no_gaussian_lr-3", "no_gaussian_lr-1", "no_gaussian_lr1"
                # "lr1_simple_batch_10_no_relax"
            ],
            labels=["1e-3", "1e-1", "1" ],
            title_tag="Deterministic Steps Learning Rate Simple",
...
if __name__ == '__main__':
    force_mut_only(plot_individually=False, gaussian=False, lr_step_only=False, lr_step_only_simple=False, compare=True)
