...
class CMAMEGA:
    ...
    def initialise_starting_parameters(
    ...
    def compute(
        ...
        self.initialise_starting_parameters(
            number_of_niches, maximum_evaluations, run_parameters, experiment_label
        )
        print("hello cma")
        solution_theta = self.crystal_system.create_one_individual(
            self.configuration_counter
        )
        ...
        while self.n_evals < maximum_evaluations:
            ...
            gradient_stack = np.stack(
                [
                    gradients[0][0][: len(solution_theta["positions"])],
                    gradients[0][1],
                    gradients[0][2],
                ]
            )
            ...
            # 9: θ′ i ← θ + ∇i
            new_atoms = [
                Atoms.fromdict(solution_theta)
                for _ in range(self.coef_optimizer.batch_size)
            ]
            for i, atom in enumerate(new_atoms):
                new_atoms[i].set_positions(
                    new_atoms[i].get_positions() + gradient_mutation_amount[i]
                )
            ...
            self.update_archive(
                population, fitness_scores, descriptors, kill_list, all_gradients
            )
            ...
            value_batch = np.asarray(
                fitness_scores
            )  # TODO: this needs to be fixed to be better
            ...
            num_parents = (
                self.coef_optimizer.batch_size // 2
            )  # todo this can be based on new solutions updated_atoms if self._selection_rule == "filter" else

            parents = [
                el.get_positions() for i, el in enumerate(updated_atoms) if i in indices
            ]
            ...
            new_positions = (
                solution_theta["positions"]
                + self.step_size_gradient_optimizer_niu * new_mean
            )
            ...
            if (
                self.coef_optimizer.check_stop(ranking_values[indices])
                or self._check_restart(new_positions)
            ).all():
                print("restarting")
