...
class CrystalSystem:
    def __init__(
        ...
        operator_probabilities: List[float] = (4.0, 2.0, 2.0, 2.0),
        ...
        self.operators = (
            self._initialise_operators(operator_probabilities)
            if alternative_operators is None
            else self._initialise_alternative_operators(
                alternative_operators, learning_rate
            )
        )
    ...
    def load_possible_pyxtal_spacegroups(self):
        if self.compound_formula is None:
            ...
            with open(
                self.main_experiment_directory.parent
    ...
    def _initialise_alternative_operators(
        self, alternative_operators, learning_rate: float
    ):
        closest_distances = closest_distances_generator(
            atom_numbers=self.atomic_numbers,
            ratio_of_covalent_radii=self.ratio_of_covalent_radii,
    ...
    def mutate(self, parents: List[Species]) -> Atoms:
        ...
        if isinstance(mutator, DQDMutationOMGMEGA) or isinstance(
            mutator, ForceMutation
        ):
